...
df <-  df %>%
  mutate(
    ...
    qbs = substr(personnelO, str_locate(personnelO, "QB")- 3,str_locate(personnelO, "QB")-2),
...
two.qb.sets <- qb.loc$fullid[which(duplicated(qb.loc[,1:2])==T)]

df = df[(df$fullid %in% two.qb.sets)==F,]
